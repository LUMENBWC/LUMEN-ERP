...
- **Máquina de estados de status** (`domain/garantir-transicao-status-valida.ts`): `RASCUNHO → ENVIADO`; `ENVIADO → APROVADO | RECUSADO | EXPIRADO`; `APROVADO → EXPIRADO`. `CONVERTIDO` existe no enum do Prisma mas **não é alcançável por nenhuma transição hoje** - reservado de propósito para o fluxo "converter em venda" da Etapa 9 (PDV), que ainda não existe. Tentar transicionar para ele (ou qualquer transição fora do grafo) lança `TransicaoStatusInvalidaError`.
...
- **`CONVERTIDO` é reservado, não implementado** - não existe hoje nenhum caminho para chegar nesse status nem uma entidade `Venda` funcional (o campo `venda` na relação do Prisma existe, mas o módulo de vendas/PDV é a Etapa 9). Quando essa etapa existir, a transição `ENVIADO/APROVADO → CONVERTIDO` deve ser adicionada ao grafo junto com o use-case que cria a venda a partir do orçamento.
