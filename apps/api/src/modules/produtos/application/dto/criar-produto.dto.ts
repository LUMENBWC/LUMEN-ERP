import { z } from 'zod';

export const UNIDADES_MEDIDA = ['UN', 'KG', 'G', 'L', 'ML', 'M', 'CX', 'PC'] as const;

export const criarProdutoSchema = z.object({
  nome: z.string().trim().min(2, 'Nome deve ter ao menos 2 caracteres.').max(255),
  descricao: z.string().trim().max(2000).nullable().optional().default(null),
  sku: z.string().trim().min(1, 'SKU é obrigatório.').max(64),
  codigoBarras: z.string().trim().min(1).max(64).nullable().optional().default(null),
  unidadeMedida: z.enum(UNIDADES_MEDIDA).default('UN'),
  categoriaId: z.string().uuid().nullable().optional().default(null),
  precoCusto: z.number().nonnegative('Preço de custo não pode ser negativo.'),
  precoVenda: z.number().nonnegative('Preço de venda não pode ser negativo.'),
  estoqueMinimo: z.number().nonnegative('Estoque mínimo não pode ser negativo.').default(0),
  ncm: z.string().trim().max(16).nullable().optional().default(null),
  cfop: z.string().trim().max(16).nullable().optional().default(null),
  cst: z.string().trim().max(16).nullable().optional().default(null),
});

export type CriarProdutoDto = z.infer<typeof criarProdutoSchema>;
