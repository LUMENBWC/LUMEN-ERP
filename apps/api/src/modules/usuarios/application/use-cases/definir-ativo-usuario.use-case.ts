...
@Injectable()
export class DefinirAtivoUsuarioUseCase {
  constructor(
  ...
  async execute(
    ...
    return this.txRunner.run(tenant.empresaId, async (tx) => {
      ...
      if (!ativo) {
        if (usuarioId === tenant.usuarioId) {
          throw new NaoPodeDesativarASiMesmoError();
        }

        const ehAdministrador = antes.papeis.some((p) => p.nome === PAPEL_ADMINISTRADOR);
        if (ehAdministrador) {
      ...
      const depois = await repo.definirAtivo(usuarioId, ativo, tenant.usuarioId);
