...
export function ContaReceberDetail({ contaReceberId }: { contaReceberId: string }) {
  ...
  if (isError || !conta)
    return <p className="text-destructive text-sm">Conta a receber não encontrada.</p>;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-muted flex flex-wrap gap-6 rounded-lg border p-4 text-sm">
        ...
        <div>
          ...
          <div className="text-lg font-semibold">R$ {conta.valorRecebido}</div>
        </div>
        <div>
      ...
      <div className="space-y-2">
        <h2 className="text-sm font-medium">Histórico de recebimentos</h2>
              ...
              <TableBody>
                {conta.recebimentos.map((recebimento) => (
                  <TableRow key={recebimento.id}>
                    <TableCell>R$ {recebimento.valor}</TableCell>
